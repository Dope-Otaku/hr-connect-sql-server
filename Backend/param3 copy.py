    ...
    try:
        ...
        for item in param_results:
            ...
            if row:
                ...
                # print(f"Object values: {object_values}")
                # print(f"Calculated product count: {product_count}")
            else:
...
def batch_insert_with_temp_table(param_ids):
    ...
    try:
        ...
        all_data = []
        start_c_num = 1
        # product_count_calculator = ProductCountCalculator()
...
def calculate_product_count(object_values):
    pc_values = [item['Value'] for item in object_values if item['Object'] == 'PRODUCT']
    temp_rem = 0
...
def collect_data_for_param(param_id, new_id, cursor, start_c_num):
    ...
    if product_count is not None:
        if pc is None:
            temp_pc = int(product_count)
        else:
            temp_pc = int(product_count) - pc
    ...
    for i in range(55):
        ...
        if is_first_set or i >= 5:
            if i == 0 and is_first_set:
                ...
                value = current_date
            elif i == 2 and is_first_set:
                value = start_time_str
            elif i == 3 and is_first_set:
                value = end_time_str
            elif i == 4 and is_first_set:
                value = shift_name
            elif i == 5:
                value = plan
            elif i == 6:
                value = str(downtime) if downtime is not None else '0'
            elif i == 7:
                value = str(product_type) if product_type is not None else '0'
            elif i == 8:
            ...
            print(f"{c_code}: {value}")

    return data, start_c_num + 55 if is_first_set else start_c_num + 50
